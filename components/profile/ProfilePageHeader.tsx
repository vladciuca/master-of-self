...
export function ProfilePageHeader({
  ...
  const handleLearnDisciplineRedirect = () => {
    if (!disabled) {
      router.push("/explore");
    }
  };
  ...
  return (
    <div className="sticky top-[68px] z-10 bg-background px-1 pb-2">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center">
          <span className="scroll-m-20 tracking-tight">Learn {title}</span>
          <Button
            onClick={handleLearnDisciplineRedirect}
            // disabled={disabled}
            variant="secondary"
            size="icon"
            className="h-8 w-8 shrink-0 rounded-full ml-3"
          >
            <Plus className="h-4 w-4" />
            <span className="sr-only">Learn {title}</span>
          </Button>
        </div>
      </div>
    </div>
