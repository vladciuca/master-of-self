...
export function ActionsField({ control, type }: ActionsFieldProps) {
  ...
  // Consider using a more efficient approach than useWatch if possible
  const actions = useWatch({
    control,
    name: "actions",
    defaultValue: [],
  ...
  const editAction = useCallback(
    ...
    [actions, setIsDrawerOpen]
  );

  // Memoize initialData to prevent unnecessary recalculations
  const initialData = useMemo(
    () => (editId ? actions.find((action) => action.id === editId) : undefined),
    [editId, actions]
  );

  const onClose = useCallback(() => setIsDrawerOpen(false), []);

  // Memoize the add action button click handler
