...
const TypeField = React.memo(({ control }: { control: any }) => (
  <FormField
    control={control}
    name="type"
    render={({ field }) => (
...
function ActionFormComponent({
  ...
  // Create a unique key for the form to force a complete reset when initialData changes
  const formKey = useMemo(
    () => initialData?.id || "new-action",
    [initialData?.id]
  );
  ...
  // Memoize the form values that are watched to prevent unnecessary re-renders
  const formType = form.watch("type");
  const formMetric = form.watch("metric");
  const dailyTargetValue = form.watch("dailyTarget");

  // Memoize the form title and description
          ...
          <div className="px-1 pb-4 space-y-4">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="flex items-center mb-6">
                    Daily {formType === "build" ? "Target" : "Limit"}:
                    <span className="ml-2 space-x-1 text-lg flex items-center">
                      <MetricIcon metric={formMetric} size={20} />
                      <span></span>
                      {dailyTargetValue}
                    </span>
                  </FormLabel>
...
export const ActionForm = React.memo(ActionFormComponent);
